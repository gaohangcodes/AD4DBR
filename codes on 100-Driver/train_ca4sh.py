...
from dataset import DataSet,DataSet_withDriverID
...
def train(logger,epoch):
    ...
    # total = 0.0
    for batch_index, batch_data in enumerate(tqdm(training_loader)):
        if args.net == 'fd':
            images, labels, labels2 = batch_data
            if args.gpu:
                labels = labels.cuda()
                images = images.cuda()
                labels2 = labels2.cuda()
                one_hot_labels = F.one_hot(labels,22)
                one_hot_labels2 = F.one_hot(labels2,args.driver_num)
                fake_label = torch.ones_like(one_hot_labels).cuda()/22
                fake_label2 = torch.ones_like(one_hot_labels2).cuda()/args.driver_num
        else:
        ...
        if args.net == 'fd':
            preds,preds1,preds2,outputs,preds4 = net(images)
        else:
        ...
        if args.net == 'fd':
            if epoch > settings.MILESTONES[1]:
                loss = F.cross_entropy(preds,labels)#/5 #* alpha1

                loss = loss + F.cross_entropy(preds1,labels2)#/5# * alpha2
                loss = loss + F.cross_entropy(outputs,labels)#/5# * alpha2
                loss = loss + F.kl_div(F.log_softmax(preds2,-1),fake_label2,reduction='batchmean')#/5# * alpha3
                loss = loss + F.kl_div(F.log_softmax(preds4,-1),fake_label,reduction='batchmean')#/5# * alpha3
            else:
                loss = F.cross_entropy(preds,labels)#/5 #* alpha1
...
if __name__ == '__main__':
    ...
    if 'my_ca_' in args.net and 'distilledfromCA' not in args.net:
        ...
        if args.dataset == 'pic-day-cam4':
            teacher_root = 'checkpoint/pic-day-cam4/split_by_driver/can/can-89-best.pth'
        elif args.dataset == 'pic-day-cam3':
            teacher_root = 'checkpoint/pic-day-cam3/split_by_driver/can/can-84-best.pth'
        elif args.dataset == 'pic-day-cam2':
            teacher_root = 'checkpoint/pic-day-cam2/split_by_driver/can/can-88-best.pth'
        elif args.dataset == 'pic-day-cam1':
            teacher_root = 'checkpoint/pic-day-cam1/split_by_driver/can/can-84-best.pth'
        elif args.dataset == 'pic-night-cam4':
            teacher_root = 'checkpoint/pic-night-cam4/split_by_driver/can/can-90-best.pth'
        elif args.dataset == 'pic-night-cam3':
            teacher_root = 'checkpoint/pic-night-cam3/split_by_driver/can/can-83-best.pth'
        elif args.dataset == 'pic-night-cam2':
            teacher_root = 'checkpoint/pic-night-cam2/split_by_driver/can/can-88-best.pth'
        elif args.dataset == 'pic-night-cam1':
            teacher_root = 'checkpoint/pic-night-cam1/split_by_driver/can/can-83-best.pth'
...
# prepare the data
    ...
    if args.net == 'fd':
        train_datasets = DataSet_withDriverID(trainloader, trainloadertxt, mean, std, flag ='train')
        args.driver_num = train_datasets.driver_num
    else:
...
# start training the model
    ...
    #best_path = 'checkpoint/pic-day-cam1/split_by_driver/ca/ca-81-best.pth'
    #best_path = args.teacher_root#'checkpoint/pic-day-cam4/split_by_driver/my_ca/my_ca-95-best.pth'
    ...
    if args.net == 'fd':
        test_args.driver_num = args.driver_num
